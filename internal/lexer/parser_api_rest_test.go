...
func TestParseApi_RouteREST(t *testing.T) {
	testCases := []struct {
		...
		{
			name:             "route allows multiple entries for errors decorator",
			input:            `POST /users/:id -> User @query(PaginationInput) @errors(404, 428)`,
			expectMethod:     "POST",
			expectPathSpec:   []string{"users", "id"},
			expectReturnType: "User",
			expectDecorators: []string{"query", "errors"},
		...
		{
			name:             "GET route with multiple error codes",
			input:            `GET /:id -> User @errors(401, 404)`,
			expectMethod:     "GET",
			expectPathSpec:   []string{"id"},
			expectReturnType: "User",
			expectDecorators: []string{"errors"},
			wantErr:          false,
		},
		{
			name:             "POST route with three error codes",
			input:            `POST / -> User @body(CreateUserInput) @errors(400, 409, 500)`,
			expectMethod:     "POST",
			expectPathSpec:   []string{},
			expectReturnType: "User",
			expectDecorators: []string{"body", "errors"},
			wantErr:          false,
		},
		{
